...
<div class="card mb-3">
    <div class="card-body py-2">
        <div class="d-flex flex-wrap align-items-center">
            ...
            <a href="<?= Controller::url('transaction') ?>" class="btn btn-sm <?= empty($currentStatus) ? 'btn-primary' : 'btn-outline-secondary' ?> mr-2 mb-1">
                Semua
            </a>
            <?php foreach ($statuses as $status => $config): ?>
            <a href="<?= Controller::url('transaction') ?>&status=<?= $status ?>" class="btn btn-sm <?= $currentStatus === $status ? 'btn-' . $config['color'] : 'btn-outline-' . $config['color'] ?> mr-2 mb-1">
...
<div class="card">
    <div class="card-header">
        ...
        <div class="card-tools">
            <a href="<?= Controller::url('transaction/create') ?>" class="btn btn-primary btn-sm">
        ...
        <table class="table table-bordered table-hover datatable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="<?= Controller::url('transaction/show/' . $trx['id']) ?>" class="btn btn-sm btn-info" title="Detail">
                            <i class="fas fa-eye"></i>
                        </a>
                        <a href="<?= Controller::url('transaction/print/' . $trx['id']) ?>" class="btn btn-sm btn-secondary" title="Cetak" target="_blank">
                            <i class="fas fa-print"></i>
                        </a>
                        <?php if ($trx['status'] !== 'selesai'): ?>
                        <a href="<?= Controller::url('transaction/next/' . $trx['id']) ?>" class="btn btn-sm btn-success" title="Lanjut Status">
